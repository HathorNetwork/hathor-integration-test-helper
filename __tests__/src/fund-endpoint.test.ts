...
import { __resetStartupForTest } from "../../src/startup";
...
import {
  ...
  handleFund,
  handleMetrics,
  __setRouteGenesisForTest,
  __resetRouteGenesisForTest,
} from "../../src/routes";
...
beforeEach(() => {
  // Pin the startup phase to a non-degraded baseline: the readiness verdict
  // reads the process-global phase, and another test file (e.g. startup.test)
  // may have left it `degraded` — Bun shares module globals across files.
  __resetStartupForTest();
  __setGenesisStateForTest({ funded: true });
  __setRouteGenesisForTest({ ready: () => true, address: () => genesisAddress });
  __setFundDepsForTest({
...
afterEach(() => {
  ...
  populateFromUtxos([]);
  __setGenesisStateForTest({ funded: null });
  __resetRouteGenesisForTest();
});
...
describe("POST /fund happy path and readiness", () => {
  ...
  test("503 SERVICE_NOT_READY when genesis has not synced", async () => {
    __setRouteGenesisForTest({ ready: () => false });
